...
exports.stripeWebhook = exports.razorpayWebhook = exports.linkTenantAccount = exports.deleteOwnerPropertyCascade = exports.confirmRazorpayPayment = exports.createRazorpayOrder = exports.lookupTenantTrustScore = exports.verifyOwnerSubscriptionPayment = exports.createOwnerSubscriptionPaymentIntent = exports.activateOwnerFreeSubscription = exports.getOwnerSubscriptionSnapshot = exports.ensureOwnerSubscriptionProfile = exports.verifyOwnerRazorpayPayment = exports.createOwnerRazorpayPaymentIntent = exports.quoteOwnerRazorpayPayment = exports.recalculateTenantBalance = exports.correctPayment = exports.deletePayment = exports.togglePayments = exports.verifyPayment = exports.createPaymentIntent = exports.quotePayment = exports.updatePaymentStatus = exports.createPayment = exports.onPaymentCreatedPaid = exports.onPaymentPaid = exports.sendRentDueReminders = exports.generateMonthlyPayments = exports.syncOwnerSummaryOnPropertyWrite = exports.syncOwnerSummaryOnTenantWrite = exports.syncOwnerSummaryOnPaymentWrite = exports.cleanupStorageOrphansAndExports = exports.cleanupExpiredWebhookEvents = void 0;
// @ts-nocheck
...
function getSecurityConfig() {
    ...
    return {
        ...
        allowedOrigin: String(cfg.allowed_origin || process.env.SECURITY_ALLOWED_ORIGIN || '').trim(),
    };
}
function assertCallableAuth(context) {
    ...
    if (enforceAppCheck && !context.app) {
        throw new functions.https.HttpsError('failed-precondition', 'App Check token is required');
    }
}
...
async function assertPaymentsEnabledOrThrow({ gateway, manual = false }) {
    ...
    if (config.maintenanceMode) {
        throw new functions.https.HttpsError('failed-precondition', 'maintenance-mode');
    }
    if (!config.paymentsEnabled) {
        throw new functions.https.HttpsError('unavailable', 'payments-disabled');
    }
    if (manual && !config.manualPaymentsEnabled) {
        throw new functions.https.HttpsError('failed-precondition', 'manual-payments-disabled');
    }
    if (String(gateway || '').toLowerCase() === 'razorpay' && !config.razorpayEnabled) {
        throw new functions.https.HttpsError('failed-precondition', 'razorpay-disabled');
    }
    return config;
}
...
async function logAdminAudit({ action, adminId, targetId = null, oldValue = null, newValue = null, reason = null, meta = null, }) {
...
async function assertTenantAccessOrThrow(uid) {
    ...
    const role = String(userDoc.data()?.role || '').trim().toLowerCase();
    if (role !== 'tenant') {
        throw new functions.https.HttpsError('permission-denied', 'Only tenant users can perform this operation');
    }
}
...
async function rateLimitOrThrow({ uid, action, limit = PAYMENT_RATE_LIMIT, windowSeconds = PAYMENT_RATE_WINDOW_SECONDS, meta = {}, }) {
    if (!uid) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
...
async function recordSecuritySignal({ type, channel, uid = null, ip = 'unknown', reason = 'unknown', statusCode = 0, meta = {}, }) {
...
function getRazorpayConfig() {
    ...
    const configuredGenericKeyId = String(cfg.key_id || process.env.RAZORPAY_KEY_ID || '').trim();
    const keyId = mode === 'live'
        ? String(cfg.live_key_id ||
            process.env.RAZORPAY_LIVE_KEY_ID ||
            (configuredGenericKeyId.startsWith('rzp_live_')
                ? configuredGenericKeyId
                : '')).trim()
        : String(cfg.test_key_id ||
            process.env.RAZORPAY_TEST_KEY_ID ||
            (configuredGenericKeyId.startsWith('rzp_test_')
                ? configuredGenericKeyId
                : 'rzp_test_SWZErkO7aPAnNO')).trim();
    const keySecret = mode === 'live'
        ? String(cfg.live_key_secret ||
            process.env.RAZORPAY_LIVE_KEY_SECRET ||
            cfg.key_secret ||
            process.env.RAZORPAY_KEY_SECRET ||
            '').trim()
        : String(cfg.test_key_secret ||
            process.env.RAZORPAY_TEST_KEY_SECRET ||
            cfg.key_secret ||
            process.env.RAZORPAY_KEY_SECRET ||
            '').trim();
    const webhookSecret = mode === 'live'
        ? String(cfg.live_webhook_secret ||
            process.env.RAZORPAY_LIVE_WEBHOOK_SECRET ||
            cfg.webhook_secret ||
            process.env.RAZORPAY_WEBHOOK_SECRET ||
            '').trim()
        : String(cfg.test_webhook_secret ||
            process.env.RAZORPAY_TEST_WEBHOOK_SECRET ||
            cfg.webhook_secret ||
            process.env.RAZORPAY_WEBHOOK_SECRET ||
            '').trim();
    const keyPrefixMismatch = (mode === 'test' && keyId.startsWith('rzp_live_')) ||
...
async function createRazorpayOrderForPayment({ paymentRef, paymentId, amountInPaise, currency, notes = {}, }) {
    const { keyId, keySecret, mode, keyPrefixMismatch } = getRazorpayConfig();
    if (keyPrefixMismatch) {
        throw new functions.https.HttpsError('failed-precondition', `Razorpay mode-key mismatch: mode=${mode}`);
    }
    if (!keyId || !keySecret) {
        throw new functions.https.HttpsError('failed-precondition', `Razorpay keys not configured for mode=${mode}`);
    }
...
function defaultPaymentFeeConfig() {
    ...
    const defaultGatewayCostPercent = parsePercent(cfg.default_gateway_cost_percent, defaultGatewayPercent);
    return {
        gstPercent,
...
async function loadPaymentFeeConfig(gateway) {
    ...
    const gatewayPercent = parsePercent(firestoreConfig?.gatewayPercents?.[safeGateway]
        || firestoreConfig?.[`${safeGateway}Percent`]
        || defaults.gatewayPercents[safeGateway]
        || defaults.defaultGatewayPercent, defaults.defaultGatewayPercent);
    const gatewayCostPercent = parsePercent(firestoreConfig?.gatewayCostPercents?.[safeGateway]
        || firestoreConfig?.[`${safeGateway}CostPercent`]
        || defaults.gatewayCostPercents[safeGateway]
        || defaults.defaultGatewayCostPercent, defaults.defaultGatewayCostPercent);
    const gstPercent = parsePercent(firestoreConfig?.gstPercent, defaults.gstPercent);
    return {
        gateway: safeGateway,
        gatewayPercent,
        gatewayCostPercent,
...
function calculateFeeBreakdownInPaise({ rentAmountInRupees, gatewayPercent, gstPercent, gatewayCostPercent, }) {
...
async function incrementFeeAnalytics({ gateway, rentAmountInPaise, convenienceFeeInPaise, totalPayableInPaise, estimatedGatewayCostInPaise, }) {
    const now = new Date();
    ...
    await analyticsRef.set({
        ...
        totalNetProfitInPaise: FieldValue.increment(Math.trunc((convenienceFeeInPaise || 0) - (estimatedGatewayCostInPaise || 0))),
...
async function getLeaseOrTenantPaymentContext({ leaseId, uid }) {
    ...
    const resolvedRentAmount = normalizeIntegerAmount(tenant.rentAmount || tenant.dueAmount || 0);
    if (!Number.isInteger(resolvedRentAmount) || resolvedRentAmount <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Calculated rent amount is invalid');
    }
    return {
...
async function deleteQueryInChunksCapped(query, { chunkSize = 400, maxDocs = 1200, } = {}) {
    let deleted = 0;
...
async function cleanupOrphanTenantDocumentFiles({ staleHours = 48, maxDocs = 250, } = {}) {
...
async function cleanupOldReportExports({ olderThanDays = 14, maxFiles = 500, } = {}) {
...
async function postWhatsAppMessage({ token, phoneNumberId, apiVersion, payload }) {
    return fetch(`https://graph.facebook.com/${apiVersion}/${phoneNumberId}/messages`, {
        method: 'POST',
        headers: {
...
async function sendWhatsAppMessage({ to, body, templateParams }) {
    const { token, phoneNumberId, apiVersion, templateName, templateLanguage, maxRetries, } = getWhatsAppConfig();
    if (!token || !phoneNumberId) {
        functions.logger.warn('WhatsApp config missing. Set whatsapp.token and whatsapp.phone_number_id.');
        return {
            ok: false,
            status: 0,
...
exports.cleanupExpiredWebhookEvents = functions
    ...
    const deletedWebhookEvents = await deleteQueryInChunksCapped(db.collection('_webhookEvents').where('expiresAt', '<=', now), { chunkSize: 300, maxDocs: 1200 });
    const deletedSecuritySignals = await deleteQueryInChunksCapped(db.collection('_securitySignals').where('expiresAt', '<=', now), { chunkSize: 300, maxDocs: 1200 });
    const deletedSecurityAlerts = await deleteQueryInChunksCapped(db.collection('_securityAlerts').where('expiresAt', '<=', now), { chunkSize: 300, maxDocs: 1200 });
    const deletedNotificationEvents = await deleteQueryInChunksCapped(db.collection('_notificationEvents').where('expiresAt', '<=', now), { chunkSize: 300, maxDocs: 1200 });
...
exports.generateMonthlyPayments = functions
    ...
    tenantsSnap.forEach((doc) => {
        const t = doc.data();
        if (!t.rentAmount || t.rentAmount <= 0)
            return;
...
async function dispatchPaymentReceivedNotification({ paymentId, payment, eventId, }) {
...
function resolveTenantOwnerLinkPolicy({ tenantData, propertyData, propertyId, actorUid, enforceOwnerActor = false, }) {
    ...
    if (isLinkedTenantProfile) {
        ...
        if (normalizedTenantPropertyName &&
...
function assertTenantBackfillProofOrThrow({ tenantData, providedToken }) {
    ...
    const storedHash = String(tenantData?.ownerAssignmentTokenHash || tenantData?.tenantLinkTokenHash || '').trim().toLowerCase();
    const storedPlain = String(tenantData?.ownerAssignmentToken || tenantData?.tenantLinkToken || '').trim();
    ...
    if (storedHash) {
        ...
        if (candidateHash !== storedHash) {
            throw new functions.https.HttpsError('permission-denied', 'tenant-link-proof-invalid');
        }
        return;
    }
    ...
    throw new functions.https.HttpsError('failed-precondition', 'tenant-link-proof-missing-on-tenant');
}
...
function buildPaymentLogPayload({ event, userId, tenantId, ownerId, paymentId, idempotencyKey, amount, method, status, errorCode, errorMessage, deviceInfo, extra = {}, }) {
    return {
...
function validatePaymentDocumentIntegrity({ payment, expectedTenantId, expectedOwnerId, expectedMethod, }) {
...
async function verifyPaymentAfterWriteOrThrow({ paymentRef, tenantId, ownerId, method, idempotencyKey, amount, }) {
...
async function blockDuplicatePaymentOrThrow({ tenantId, amount, method, idempotencyKey, }) {
...
exports.createPayment = functions.region('asia-south1')
...
exports.updatePaymentStatus = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    // assertEmailVerifiedOrThrow(context);
    const uid = context.auth.uid;
...
exports.quotePayment = functions.https.onCall(async (data, context) => {
    ...
    if (gateway === 'cashfree') {
        throw new functions.https.HttpsError('failed-precondition', 'Cashfree has been removed from this project. Use Razorpay.');
    }
    ...
    if (enteredRentAmountInRupees < 0 || !Number.isFinite(enteredRentAmountInRupees)) {
        throw new functions.https.HttpsError('invalid-argument', 'enteredRentAmountInRupees must be a positive number when provided');
    }
    ...
    if (lease.tenantId && !(await leaseBelongsToTenant(lease, context.auth.uid))) {
        throw new functions.https.HttpsError('permission-denied', 'Lease does not belong to tenant');
    }
    if (lease.status && lease.status !== 'active') {
        throw new functions.https.HttpsError('failed-precondition', 'Lease is not active');
    }
    ...
    if (feeBreakdown.convenienceFeeInPaise < feeBreakdown.estimatedGatewayCostInPaise) {
        throw new functions.https.HttpsError('failed-precondition', 'Configured fee is below gateway cost. Refusing quote to prevent platform loss.');
    }
    return {
...
exports.createPaymentIntent = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!leaseId || !idempotencyKey) {
            throw new functions.https.HttpsError('invalid-argument', 'leaseId and idempotencyKey are required');
        }
        const paymentContext = await getLeaseOrTenantPaymentContext({
            leaseId,
            uid: context.auth.uid,
        ...
        if (existingByIdempotency) {
            ...
            if (existingTenantId !== String(lease.tenantId || context.auth.uid).trim() ||
        ...
        if (feeBreakdown.convenienceFeeInPaise < feeBreakdown.estimatedGatewayCostInPaise) {
            throw new functions.https.HttpsError('failed-precondition', 'Configured fee is below gateway cost. Refusing payment intent to prevent platform loss.');
        }
        ...
        if (existingPayment.exists) {
            ...
            if (currentStatus === 'paid' || currentStatus === 'success') {
                throw new functions.https.HttpsError('failed-precondition', 'Payment already completed for this period');
            }
        }
        ...
        if (existingTransaction.exists) {
            ...
            return {
                ...
                estimatedGatewayCostInPaise: Number(payment.get('estimatedGatewayCostInPaise') || feeBreakdown.estimatedGatewayCostInPaise),
        ...
        if (gateway === 'stripe') {
            ...
            if (!secretKey) {
                throw new functions.https.HttpsError('failed-precondition', 'Stripe keys not configured');
            }
            throw new functions.https.HttpsError('failed-precondition', 'Stripe intent creation not configured');
        }
        if (gateway === 'cashfree') {
            throw new functions.https.HttpsError('failed-precondition', 'Cashfree has been removed from this project. Use Razorpay.');
        }
        throw new functions.https.HttpsError('invalid-argument', 'Unsupported payment gateway');
    }
    catch (error) {
        const errorCode = String(error?.code || 'internal');
        const errorMessage = error?.message || String(error);
...
exports.verifyPayment = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!paymentId || !gateway) {
            throw new functions.https.HttpsError('invalid-argument', 'paymentId and gateway are required');
        }
        const paymentRef = db.collection('payments').doc(paymentId);
        ...
        if (!ownsPayment) {
            throw new functions.https.HttpsError('permission-denied', 'Payment does not belong to current user');
        }
        const transactionId = payment.transactionId || payload.transactionId;
        if (!transactionId) {
            throw new functions.https.HttpsError('failed-precondition', 'Transaction not found for payment');
        }
        ...
        if (gateway === 'razorpay') {
            ...
            if (!keySecret) {
                throw new functions.https.HttpsError('failed-precondition', `Razorpay secret not configured for mode=${mode}`);
            }
            ...
            if (!razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
                throw new functions.https.HttpsError('invalid-argument', 'Missing Razorpay verification data');
            }
            const expected = crypto
            ...
            const finalized = await db.runTransaction(async (t) => {
                const txSnap = await t.get(transactionRef);
                if (txSnap.exists && txSnap.get('status') === 'success')
                    return false;
        ...
        if (gateway === 'stripe') {
            throw new functions.https.HttpsError('failed-precondition', 'Stripe verification not configured');
        }
        if (gateway === 'cashfree') {
            throw new functions.https.HttpsError('failed-precondition', 'Cashfree has been removed from this project. Use Razorpay.');
        }
...
// ==========================================================
exports.togglePayments = functions.region('asia-south1').https.onCall(async (data, context) => {
    assertCallableAuth(context);
...
exports.deletePayment = functions.region('asia-south1').https.onCall(async (data, context) => {
    ...
    const reason = String(data?.reason || '').trim();
    if (!paymentId || !reason) {
        throw new functions.https.HttpsError('invalid-argument', 'paymentId and reason are required');
    }
    const paymentRef = db.collection('payments').doc(paymentId);
    const paymentSnap = await paymentRef.get();
    if (!paymentSnap.exists) {
...
exports.correctPayment = functions.region('asia-south1').https.onCall(async (data, context) => {
    assertCallableAuth(context);
    await assertAdminAccessOrThrow(context.auth.uid);
    const paymentId = String(data?.paymentId || '').trim();
    ...
    if (!paymentId || !reason || typeof updates !== 'object') {
        throw new functions.https.HttpsError('invalid-argument', 'paymentId, updates, and reason are required');
    }
    const paymentRef = db.collection('payments').doc(paymentId);
    let oldValue = null;
    let newValue = null;
    await db.runTransaction(async (txn) => {
        ...
        if (paidAmount < 0 || remainingAmount < 0 || (paidAmount + remainingAmount) !== baseAmount) {
            throw new functions.https.HttpsError('invalid-argument', 'Invalid payment correction values');
        }
...
exports.recalculateTenantBalance = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    await assertAdminAccessOrThrow(context.auth.uid);
    const tenantId = String(data?.tenantId || '').trim();
...
// ==========================================================
exports.quoteOwnerRazorpayPayment = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    ...
    if (!Number.isInteger(amount) || amount <= 0) {
        throw new functions.https.HttpsError('invalid-argument', 'amount must be a positive integer in rupees');
    }
    if (amount > 5000000) {
        throw new functions.https.HttpsError('invalid-argument', 'Amount exceeds allowed maximum');
    }
...
exports.createOwnerRazorpayPaymentIntent = functions.region('asia-south1').https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
    await assertOwnerAccessOrThrow(ownerId);
    ...
    if (!tenantId || !propertyId || !idempotencyKey || !Number.isInteger(amount) || amount <= 0) {
        throw new functions.https.HttpsError('invalid-argument', 'tenantId, propertyId, idempotencyKey and positive integer amount are required');
    }
    if (amount > 5000000) {
        throw new functions.https.HttpsError('invalid-argument', 'Amount exceeds allowed maximum');
    }
    ...
    if (existingByIdempotency) {
        ...
        return {
            paymentId: String(existingData.paymentId || existingByIdempotency.id).trim(),
            gateway: 'razorpay',
            amountInPaise: Number(existingData.amountInPaise
                || existingData.totalPayableInPaise
                || feeBreakdown.totalPayableInPaise),
            rentAmountInPaise: Number(existingData.rentAmountInPaise || feeBreakdown.rentAmountInPaise),
            convenienceFeeInPaise: Number(existingData.convenienceFeeInPaise || feeBreakdown.convenienceFeeInPaise),
            totalPayableInPaise: Number(existingData.totalPayableInPaise || feeBreakdown.totalPayableInPaise),
        ...
        if (existingPaymentId) {
            ...
            if (existingPayment.exists) {
                ...
                return {
                    paymentId: existingPaymentId,
                    gateway: 'razorpay',
                    amountInPaise: Number(existingPayment.get('amountInPaise')
                        || existingPayment.get('totalPayableInPaise')
                        || feeBreakdown.totalPayableInPaise),
                    rentAmountInPaise: Number(existingPayment.get('rentAmountInPaise') || feeBreakdown.rentAmountInPaise),
                    convenienceFeeInPaise: Number(existingPayment.get('convenienceFeeInPaise') || feeBreakdown.convenienceFeeInPaise),
                    totalPayableInPaise: Number(existingPayment.get('totalPayableInPaise') || feeBreakdown.totalPayableInPaise),
...
exports.verifyOwnerRazorpayPayment = functions.region('asia-south1').https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
    await assertOwnerAccessOrThrow(ownerId);
    const paymentId = String(data?.paymentId || '').trim();
    ...
    if (!paymentId || !razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
        throw new functions.https.HttpsError('invalid-argument', 'paymentId and Razorpay payload (orderId, paymentId, signature) are required');
    }
    const paymentRef = db.collection('payments').doc(paymentId);
    const paymentSnap = await paymentRef.get();
    if (!paymentSnap.exists) {
    ...
    const finalized = await db.runTransaction(async (txn) => {
        ...
        const convenienceFeeInPaise = Number(payment.convenienceFeeInPaise || 0);
        const totalPayableInPaise = Number(payment.totalPayableInPaise
    ...
    if (finalized) {
        await incrementFeeAnalytics({
            ...
            convenienceFeeInPaise: Number(payment.convenienceFeeInPaise || 0),
            totalPayableInPaise: Number(payment.totalPayableInPaise
                || payment.amountInPaise
...
async function assertOwnerAccessOrThrow(ownerId) {
    ...
    if (ownerRole && ownerRole !== 'owner') {
        throw new functions.https.HttpsError('permission-denied', 'Only owners can perform this operation');
    }
}
...
exports.ensureOwnerSubscriptionProfile = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
...
exports.getOwnerSubscriptionSnapshot = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
    const requestedOwnerId = String(data?.ownerId || ownerId).trim();
...
exports.activateOwnerFreeSubscription = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
    const requestedOwnerId = String(data?.ownerId || ownerId).trim();
...
exports.createOwnerSubscriptionPaymentIntent = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    throw new functions.https.HttpsError('failed-precondition', 'Cashfree subscription flow has been removed from this project.');
});
exports.verifyOwnerSubscriptionPayment = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    throw new functions.https.HttpsError('failed-precondition', 'Cashfree subscription flow has been removed from this project.');
});
...
exports.lookupTenantTrustScore = functions.region('asia-south1')
    ...
    await db.runTransaction(async (txn) => {
        ...
        if (currentCount >= 30) {
            throw new functions.https.HttpsError('resource-exhausted', 'Daily lookup limit reached (30/day).');
        }
        txn.set(quotaRef, {
            ownerId,
            dayKey,
...
// ==========================================================
exports.createRazorpayOrder = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    await assertTenantAccessOrThrow(context.auth.uid);
    const { keyId, keySecret, mode, keyPrefixMismatch } = getRazorpayConfig();
    ...
    if (!paymentId || !amount || amount <= 0) {
        throw new functions.https.HttpsError('invalid-argument', 'paymentId and amount are required');
    }
    const paymentRef = db.collection('payments').doc(paymentId);
    const paymentSnap = await paymentRef.get();
    if (!paymentSnap.exists) {
    ...
    if (!Number.isFinite(expectedAmount) || expectedAmount <= 0) {
        throw new functions.https.HttpsError('failed-precondition', 'Payment amount is invalid on server');
    }
    if (amount !== expectedAmount * 100) {
        throw new functions.https.HttpsError('invalid-argument', 'Amount mismatch with server payment record');
    }
    const auth = Buffer.from(`${keyId}:${keySecret}`).toString('base64');
    const orderRes = await fetch('https://api.razorpay.com/v1/orders', {
        method: 'POST',
...
exports.confirmRazorpayPayment = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    await assertTenantAccessOrThrow(context.auth.uid);
    const { keySecret, mode } = getRazorpayConfig();
    ...
    if (!storedOrderId || storedOrderId !== razorpayOrderId) {
        throw new functions.https.HttpsError('failed-precondition', 'Order id does not match server payment record');
    }
    await paymentRef.set({
        status: 'paid',
        method: 'online',
...
exports.deleteOwnerPropertyCascade = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const ownerId = context.auth.uid;
    const propertyId = String(data?.propertyId || '').trim();
    ...
    if (role !== 'owner') {
        throw new functions.https.HttpsError('permission-denied', 'Only owners can delete properties');
    }
    const propertyRef = db.collection('properties').doc(propertyId);
    ...
    if (propertyOwnerId && propertyOwnerId !== ownerId) {
        throw new functions.https.HttpsError('permission-denied', 'Property does not belong to current owner');
    }
    if (!propertyOwnerId) {
        ...
        if (ownershipProbe.empty) {
            throw new functions.https.HttpsError('permission-denied', 'Unable to verify property ownership');
        }
    }
    await deleteQueryInChunks(db.collection('tenants').where('propertyId', '==', propertyId));
    await deleteQueryInChunks(db.collection('payments').where('propertyId', '==', propertyId));
    await deleteQueryInChunks(db.collection('leases').where('propertyId', '==', propertyId));
    await propertyRef.delete();
    return { ok: true, deleted: true };
});
exports.linkTenantAccount = functions.region('asia-south1')
    .https.onCall(async (data, context) => {
    assertCallableAuth(context);
    const uid = context.auth.uid;
    ...
    if (!email) {
        throw new functions.https.HttpsError('failed-precondition', 'Authenticated tenant must have an email');
    }
...
exports.razorpayWebhook = functions.region('asia-south1')
