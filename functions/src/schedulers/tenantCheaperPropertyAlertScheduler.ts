...
import { normalizeCityKey } from "../utils/cityKey";
...
const estimatePropertyRent = (data: FirebaseFirestore.DocumentData): number => {
  const direct = asInt(
    data.rentAmount ??
      data.monthlyRent ??
      data.startingRent ??
...
const findCheaperCandidate = async (
  cityKey: string,
  tenantRent: number,
): Promise<{ rent: number; propertyId: string } | null> => {
  ...
  let best: { rent: number; propertyId: string } | null = null;
...
const isNotificationsEnabled = async (userId: string): Promise<boolean> => {
...
export const sendTenantCheaperPropertyAlerts = onSchedule(
  {
    schedule: "0 */6 * * *",
    ...
    try {
      ...
      for (const tenantDoc of tenantsSnapshot.docs) {
        ...
        try {
          ...
          const payload = {
            type: "CHEAPER_PROPERTY_ALERT" as const,
          ...
          logError("sendTenantCheaperPropertyAlerts tenant iteration failed", {
      ...
      logError("sendTenantCheaperPropertyAlerts failed", {
        dateBucket,
